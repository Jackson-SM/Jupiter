...
async function handleSubmit(event) {
  ...
  const data = add_member({email: formObject.email, projectId: formObject.projectId});
}
...
const add_member = async (data) => {
  ...
  if(response.status){
    switch(response.status){
      ...
      case 401:
        return createNotification("Você não possui permissão para adicionar um membro.")
      case 409:
